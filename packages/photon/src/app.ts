...
import { registry } from "./modifiers/registry.ts";
import { type OnboardInPhoton, onboardModifier } from "./modifiers/onboard.ts";
import { type SendInPhoton, sendModifier } from "./modifiers/send.ts";

declare module "./app.ts" {
    interface App<Name extends string, Description extends string, Photon extends {} = {}> {
        onboard(
            this: Photon extends OnboardInPhoton ? App<Name, Description, Photon> : never,
        ): App<Name, Description, ReturnWithUnique<Photon, ReturnType<typeof onboardModifier>>>;

        send(
            this: Photon extends SendInPhoton ? App<Name, Description, Photon> : never,
            content: string,
...
for (const key of Object.keys(registry)) {
    const name = key as keyof typeof registry;
    const modifierData = registry[name];

    (App.prototype as any)[name] = function (...args: any[]) {
        const modifierFn = modifierData.func as (...args: any[]) => any;
        const modifierInstance = modifierFn(...args);

        if (modifierData.type === "base") {
            return (this as any).baseModifier(modifierInstance);
        }
        return (this as any).modifier(modifierInstance);
    };
}
